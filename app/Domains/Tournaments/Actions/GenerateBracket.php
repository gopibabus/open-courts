    ...
    public function handle(TournamentCategory $category): void
    {
        DB::transaction(function () use ($category): void {
            ...
            foreach ($byRound[1] as $pos => $match) {
                $match->update([
                    'player_one_id' => $entrants[$seedOrder[$pos * 2] - 1] ?? null,
                    'player_two_id' => $entrants[$seedOrder[$pos * 2 + 1] - 1] ?? null,
                ]);
            }

            // Auto-advance byes — a round-1 match with exactly one player present.
            foreach ($byRound[1] as $match) {
                $match->refresh();
                $one = $match->player_one_id;
                $two = $match->player_two_id;
                if (($one === null) !== ($two === null)) {
                    $this->advanceBye($match, (int) ($one ?? $two));
                }
            }
        });
    }

    private function advanceBye(TournamentMatch $match, int $winnerId): void
    {
        $match->update([
            'winner_id' => $winnerId,
            'status' => 'completed',
