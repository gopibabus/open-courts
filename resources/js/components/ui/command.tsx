...
/**
 * shadcn/ui's Command component — a thin wrapper around `cmdk`
 * (github.com/pacocoursey/cmdk). `CommandDialog` mounts it inside our Dialog with the
 * default close button hidden and padding stripped, so it reads as a command palette.
 */
...
const CommandDialog = ({ children, ...props }: DialogProps) => (
    <Dialog {...props}>
        <DialogContent className="overflow-hidden p-0 gap-0 [&>button:last-child]:hidden">
            <DialogTitle className="sr-only">Command palette</DialogTitle>
            <DialogDescription className="sr-only">Search for a page and jump to it.</DialogDescription>
...
>(({ className, ...props }, ref) => (
    <CommandPrimitive.Group ref={ref} className={cn('text-foreground overflow-hidden p-1', className)} {...props} />
));
